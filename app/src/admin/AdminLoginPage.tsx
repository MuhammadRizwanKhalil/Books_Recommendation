...
import { Eye, EyeOff, ShieldCheck, Mail, Lock, Loader2, KeyRound } from 'lucide-react';
...
export function AdminLoginPage({ onSuccess, onCancel }: AdminLoginPageProps) {
  const { signIn, verify2FA, signOut, isAdmin, isAuthenticated, user } = useAuth();
  ...
  const handle2FASubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const success = await verify2FA(tempToken, twoFACode);
      if (success) {
        setTimeout(() => {
          const storedUser = localStorage.getItem('thebooktimes-user');
          if (storedUser) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-muted/30 p-4">
      ...
      <Card className="w-full max-w-md relative z-10 shadow-2xl border-border/50">
        <CardHeader className="text-center space-y-4 pb-2">
          ...
          <div>
            <CardTitle className="text-2xl font-bold tracking-tight">
              {needs2FA ? 'Two-Factor Authentication' : 'Admin Panel'}
            </CardTitle>
        ...
        <CardContent className="pt-4">
          ...
          {isAuthenticated && !isAdmin && !needs2FA && (
            ...
            <form onSubmit={handle2FASubmit} className="space-y-4">
              <div className="p-3 rounded-lg bg-muted/50 mb-4">
                <p className="text-sm text-muted-foreground">
                  Signing in as <strong>{twoFAEmail}</strong>
                </p>
              </div>
