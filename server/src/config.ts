...
export const config = {
  ...
  importJob: {
    ...
    dailyBooksPerQuery: Math.max(1, parseInt(process.env.IMPORT_DAILY_PER_QUERY || '80', 10) || 80),
    dailyTargetBooks: Math.max(1, parseInt(process.env.IMPORT_DAILY_TARGET_BOOKS || '500', 10) || 500),
    fullImportTargetBooks: Math.max(1, parseInt(process.env.IMPORT_FULL_TARGET_BOOKS || '1000', 10) || 1000),
    googleDailyRequestBudget: Math.max(1, parseInt(process.env.IMPORT_GOOGLE_DAILY_REQUEST_BUDGET || '900', 10) || 900),
