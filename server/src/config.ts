...
export const config = {
  ...
  importJob: {
    ...
    dailyBooksPerQuery: Math.max(40, parseInt(process.env.IMPORT_DAILY_PER_QUERY || '80', 10) || 80),
    dailyTargetBooks: Math.max(500, parseInt(process.env.IMPORT_DAILY_TARGET_BOOKS || '500', 10) || 500),
    fullImportTargetBooks: Math.max(1000, parseInt(process.env.IMPORT_FULL_TARGET_BOOKS || '1000', 10) || 1000),
    googleDailyRequestBudget: Math.max(900, parseInt(process.env.IMPORT_GOOGLE_DAILY_REQUEST_BUDGET || '900', 10) || 900),
    googleRequestDelayMs: Math.max(0, parseInt(process.env.IMPORT_GOOGLE_REQUEST_DELAY_MS || '500', 10) || 500),
  },
